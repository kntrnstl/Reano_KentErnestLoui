...
<head>
  ...
  <style>
    ...
    /* Pagination layout / appearance fixes */
    .pagination,
    .pager,
    ul.pagination,
    nav.pager {
...
<body class="min-h-screen flex flex-col items-center p-6 font-sans text-gray-900 text-lg">
  ...
  <h1 class="text-5xl font-bold mb-10 text-indigo-600 animate-fadeInUp">Student Information System</h1>
  ...
  <div class="w-full max-w-5xl flex flex-col sm:flex-row sm:justify-between sm:items-center mb-8 gap-4 animate-fadeInUp">
    <form method="get" action="<?= site_url() ?>" class="flex w-full sm:w-auto">
      ...
      <button type="submit" class="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-3 rounded-r-lg shadow transition-all duration-300 transform hover:scale-110 text-lg">🔍</button>
    </form>

    <a href="<?= site_url('/users/create'); ?>" class="bg-indigo-600 text-white px-8 py-3 rounded-md font-semibold shadow-md hover:bg-indigo-700 transition transform hover:scale-110 text-center text-lg">
      + Create Record
    </a>
  </div>
  ...
  <div class="overflow-x-auto w-full max-w-5xl bg-white rounded-lg shadow-lg animate-fadeInUp">
    <table class="min-w-full text-lg border-collapse">
      ...
      <tbody>
  <?php 
    $rowNumber = 1; // always reset to 1 per page
